...
void fill_up_edge_vector( Square& rec,
...
void eachThreadGenEdgesUsingMutex( std::vector<Square>& rectangles,
	...
	for( unsigned int recIdx = threadIdx; recIdx < rectangles.size(); recIdx += nThreads ) {
		Square& rec = rectangles.at(recIdx);
...
void eachThreadGenEdgesUsingQueues(	threadsafe_queue<Square>& rec_queue,
...
bool GraphGen_notSorted::GenerateGraph(	const unsigned long long nEdges,
	...
	#ifdef USE_A_MUTEX_TO_CONTROL_WRITE_TO_FILE_INSTEAD_OF_CONCURRENT_QUEUES
